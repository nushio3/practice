...
evalB op a1 b1 =
  case (a1,b1) of
   ...
   (Branch _, Branch _) -> error "branch length mismatch" -- This portion obviously requires metadata.
   (Value x, ys) -> eval1 (op x) ys
   (xs, Value y) -> eval1 (flip op y) xs
   (Value x, Value y) -> Value (op x y) -- Here, metadata should be merged.

{- Do we have a unique rule with which metadata can be handled?
or, do we need to manage metadata separately ? -}
