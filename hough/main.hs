...
main = do
  ...
  let p1 = lines str
      ...
      pts :: [(Double, Double)]
      pts = map (\(y,(x,_)) -> (fromIntegral x, fromIntegral y)) p2


  let dx :: Double ; xsize :: Int
      dx = 0.2
      xsize = ceiling $ (/dx) $ fromIntegral $ maximum $ map length p1
  let dy :: Double ; ysize :: Int
      dy = 0.2
      ysize = ceiling $ (/dy) $ fromIntegral $ length p1
